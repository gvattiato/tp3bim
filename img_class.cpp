...
img::img(void) {
  width = 0;
  height = 0;
  data = new u_char [1]; //Allocation of memory to be freed once the img is deleted
}

img::img(const img& original) {
  width = original.getWidth();
  height = original.getHeight();
  data = new u_char [3*width*height];
  memcpy(data, original.getData(), 3 * width * height * sizeof(*data));
}
...
void img::ppm_write_to_file(const char* fileName)
{
...
void img::ppm_read_from_file(const char* fileName)
{
